...
candidates as (
  select p.adam, p.title, p.authority_name, p.publication_date, p.document_category,
...
select n, authority_name, norm_title, adams, categories, dates
from grouped
where n > 1
...
grouped as (
  select authority_name, budget, norm_title, count(*) as n
  from candidates
  group by authority_name, budget, norm_title
)
select
  (select count(*) from candidates) as total_candidates,
  (select count(*) from grouped where n > 1) as duplicate_groups,
  (select coalesce(sum(n), 0) from grouped where n > 1) as notices_in_groups;
