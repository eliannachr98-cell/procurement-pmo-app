...
//
// Runs once per team (see public.teams / sql/teams.sql) - each team has its
// own watchlist/region filter/recipients, isolated from every other team's,
// so a recipient in one team never gets candidates matched against another
// team's picks.
export async function GET(request: Request) {
  ...
  try {
    ...
    const teams = await supabaseGet<TeamRow[]>("teams?select=id,name");
    if (!teams.length) return NextResponse.json({ sent: false, reason: "no teams" });
    ...
    for (const team of teams) {
      const [watchlist, nutsFilter, recipients] = await Promise.all([
        ...
        supabaseGet<RecipientRow[]>(`alert_recipients?select=email&team_id=eq.${team.id}`),
      ]);

      if (!watchlist.length || !recipients.length) {
        perTeam.push({ team: team.name, sent: false, reason: !watchlist.length ? "no CPVs on the watchlist" : "no email recipients" });
        continue;
      }
      ...
      perTeam.push({ team: team.name, sent: sentCount > 0, recipientsEmailed: sentCount, itemsSent, failedRecipients });
    }

    return NextResponse.json({ teams: perTeam });
  } catch (error) {
